...
def main(args):
   ...
   for n in tqdm(range(ne+1)[1:]):  
      ...
      for l in range(max_l+1):
         ...
         vpot = [epsilon * (-2.0*pow(sigma/r_i,6) + pow(sigma/r_i,12)) for r_i in r]

         f =  [1.0 - ddx12/hbar2m * ( hbar2m*l*(l+1)/pow(r_i,2) + vpot_i - e ) for (r_i,vpot_i) in zip(r,vpot)]
         u = zeros(malha+1)
         pts_iniciais (r, u)
         ...
         secao[l] = secao[l] + 4*pi/(q*q) * (2*l+1)*pow(sin(delta),2)
