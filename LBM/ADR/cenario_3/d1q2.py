...
# LEITURA DO YAML
# ==========================================================
with open("d1q2.yaml", "r") as file:   # ← Alterado para d1q2.yaml
...
print("Configuração carregada do d1q2.yaml:")
print(f"Nx = {Nx} | Lx = {Lx}")
...
# ==========================================================
c = np.array([-1, 1])           # ← D1Q2
w = np.array([0.5, 0.5])        # ← Pesos para D1Q2
Q = 2
...
for it in tqdm(range(maxIter)):
    ...
    # Streaming D1Q2
    f[0] = np.roll(f[0], -1)   # direção positiva (c = +1)
    f[1] = np.roll(f[1],  1)   # direção negativa (c = -1)  ← corrigido índice
...
print("\n" + "="*70)
print("LBM D1Q2 - ADR 1D Steady State (MMS)")
print("="*70)
...
plt.plot(x, phi, 'r--', linewidth=2, label="LBM D1Q2")
plt.title("LBM D1Q2 vs Solução Exata\nEquação ADR 1D Steady-State")
